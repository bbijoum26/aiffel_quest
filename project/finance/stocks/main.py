...
def data_preprocessing(stock_data):
    ...
    if 'change' in stock_data.columns:
        stock_['forecast'] = stock_['close'].shift(-forecast_out)
    else:
        # 'change' 컬럼이 없는 경우, 다른 방식으로 X 변수를 선택하거나 처리
        pass
...
def randomforest():
    ...
    train_score_label = Label(score_frame, text=f'Train Score: {train_score}')
    train_score_label.pack()
    
    # 테스트 데이터에 대한 정확도 출력
    test_score = model.score(X_test, y_test)
    test_score_label = Label(score_frame, text=f'Test Score: {test_score}')
...
score_clear = tk.Button(modelling_frame, text='Clear', width=10, command=socre_clear)
score_clear.pack(padx=5, pady=10)
