...
def load_data():
    ...
    model.load_weights(f'{model_path}/autoInt_model_weights_plus.h5')

    label_encoders = joblib.load(f'{data_path}/label_encoders_plus.pkl')
...
def predict_model(model, input_data):
    # input_data는 모든 필수 특징을 포함하는 DataFrame이라고 가정
    # 모델 예측을 위해 DataFrame을 특징 값 리스트로 변환
    feature_values = [input_data[col].values for col in input_data.columns]
    
    # 예측 수행
    predictions = model.predict(feature_values)
    
    # 상위 추천 결과 얻기; 필요에 따라 조정
    top_indices = np.argsort(-predictions.flatten())[:10]  # 필요에 따라 상위 추천 개수 조정
    recom_top = [(index, predictions[index]) for index in top_indices]
    return recom_top

# get_recom 함수를 기존 Streamlit 애플리케이션 구조에 맞게 업데이트
def get_recom(user, user_non_seen_dict, user_df, movies_df, r_year, r_month, model, label_encoders):
    '''
    AutoInt+ TensorFlow 모델과 함께 작동하도록 함수 업데이트.
    '''
    user_non_seen_movie = user_non_seen_dict.get(user)
    user_id_list = [user for _ in range(len(user_non_seen_movie))]
    r_decade = str(r_year - (r_year % 10)) + 's'
    ...
    merge_data = pd.concat([user_info, user_non_seen_movie], axis=1)
    merge_data.fillna('no', inplace=True)
    merge_data = merge_data[['user_id', 'movie_id','movie_decade', 'movie_year', 'rating_year', 'rating_month', 'rating_decade', 'genre1','genre2', 'genre3', 'gender', 'age', 'occupation', 'zip']]
    
    for col, le in label_encoders.items():
        if col in merge_data:
            merge_data[col] = le.transform(merge_data[col])
    
    recom_top = predict_model(model, merge_data)
    origin_m_id = [int(label_encoders['movie_id'].inverse_transform([r[0]])[0]) for r in recom_top]
    return movies_df[movies_df['movie_id'].isin(origin_m_id)]
